/**
 * Claude Usage Stick — Fikra ES3C28P (ESP32-S3), com touch
 * Placa: módulo de display 2.8" da Fikra Creative Studio (fabricante
 * ShenZhen QDtech / LCDWIKI), ESP32-S3 com 8MB PSRAM OPI, display ILI9341V
 * SPI 240x320 (rotacionado p/ landscape), touch capacitivo FT6336G I2C.
 * Lógica de negócio/UI compartilhada com o CYD via firmware/lib_core/ —
 * ver firmware/README.md. Pinos conforme Manual Técnico Fikra 2.0.
 *
...
SPIClass sharedSPI(FSPI);   // display — ESP32-S3 não tem VSPI/HSPI; touch é I2C, sem contenção de barramento
static FT6336_TouchDrv g_touch(TOUCH_SDA, TOUCH_SCL, TOUCH_RST, TOUCH_INT, TOUCH_ADDR, TFT_ROTATION);
WiFiManager g_wifi;
...
// ============================================================
// Pipeline de display — sem Canvas, LVGL desenha direto no painel via
// draw16bitRGBBitmap (render parcial, sem PSRAM). Ver firmware/claude_stick_cyd/
// pro padrão original (mesmo driver Arduino_GFX + ILI9341).
// ============================================================
...
static void render_state() {
  ...
  switch (g_state) {
    ...
    case ST_EXTRA:     ui_rgb(); break;   // LED RGB (hardware específico da Fikra)
    case ST_ERROR: {
...
void setup() {
  Serial.begin(115200);
  delay(300);
  Serial.println("\n=== Claude Usage Stick — Fikra ES3C28P (touch) ===");

  // Display — ILI9341 via SPI padrão (não compartilhado com touch, que é I2C)
  ...
  // invertDisplay(true) validado como correto pro painel da Fikra (false
  // deixa a tela toda branca) — ajustar aqui se as cores saírem trocadas.
  gfx->invertDisplay(true);
  // Preto puro (0,0,0) renderiza correto nesse painel; cinza bem escuro
  // (ex.: C_BG=0x0F0F12) saía mais claro que devia — tentativas de VCOM
  // e curva de gamma (0x26) não mudaram nada. Solução: C_BG virou preto
  // puro em ui_helpers.h (compartilhado com o CYD, que também fica bem
  // com preto puro).
  ...
  // LVGL — render parcial em RAM comum (perfil conservador — ver lv_conf.h)
  lv_init();
  ...
  // LED RGB WS2812 — expõe a entrada "LED RGB" em Ajustes (ver ST_EXTRA).
  // Rótulo fixo (não usa TRS) pra não depender de g_lang, que só é
  // carregado logo abaixo em load_persisted().
  led_init();
  g_hasBoardExtra = true;
  g_boardExtraLabel = LV_SYMBOL_SETTINGS "  LED RGB";
